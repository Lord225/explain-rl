...
from stable_baselines3.common.buffers import BaseBuffer, RolloutBuffer, DictReplayBuffer
...
class CustomRolloutBuffer(BaseBuffer):
    ...
    def add(
        ...
        self.seg_obsevations[self.pos] = np.array(seg_obs)
        self.pos += 1
    ...
    def _get_samples(
        ...
        data = (
            ...
            self.seg_obsevations[batch_inds],
        )
        return CustomRolloutBufferSamples(*tuple(map(self.to_torch, data)))
    


class CustomSegmentedRolloutBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    ...
    def reset(self) -> None:
        ...
        self.advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.next_observations = np.zeros((self.buffer_size, self.n_envs, *self.obs_shape), dtype=np.float32)
        self.seg_obsevations = np.zeros((self.buffer_size, self.n_envs, 16, 16, 1), dtype=np.int64)
        self.generator_ready = False
        super().reset()
    ...
    def add(
        ...
        self.log_probs[self.pos] = log_prob.clone().cpu().numpy()
        self.next_observations[self.pos] = np.array(next_obs)
        self.seg_obsevations[self.pos] = np.array(seg_obs, dtype=np.int64)
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
...
SelfOnPolicyAlgorithm = TypeVar("SelfOnPolicyAlgorithm", bound="CustomOnPolicyAlgorithm")
...
class CustomOnPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        use_sde: bool,
        sde_sample_freq: int,
        rollout_buffer_class: Optional[type[CustomRolloutBuffer]] = None,
    ...
    def _setup_model(self) -> None:
        ...
        if self.rollout_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                ...
                self.rollout_buffer_class = CustomRolloutBuffer
...
class CustomSegmentedOnPolicyAlgorithm(BaseAlgorithm):
    """
    The base for On-Policy algorithms (ex: A2C/PPO).
    ...
    rollout_buffer: CustomSegmentedRolloutBuffer
    policy: ActorCriticPolicy
    ...
    def __init__(
        ...
        use_sde: bool,
        sde_sample_freq: int,
        rollout_buffer_class: Optional[type[CustomSegmentedRolloutBuffer]] = None,
        rollout_buffer_kwargs: Optional[dict[str, Any]] = None,
        stats_window_size: int = 100,
        tensorboard_log: Optional[str] = None,
        monitor_wrapper: bool = True,
    ...
    def _setup_model(self) -> None:
        ...
        if self.rollout_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                ...
                self.rollout_buffer_class = CustomSegmentedRolloutBuffer
...
class CustomPPOLinear(CustomSegmentedOnPolicyAlgorithm):
    policy_aliases: ClassVar[dict[str, type[BasePolicy]]] = {
        "MlpPolicy": ActorCriticPolicy,
        "CnnPolicy": ActorCriticCnnPolicy,
        "MultiInputPolicy": MultiInputActorCriticPolicy,
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                segments = rollout_data.seg_obsevations # int32, index of segmentation
                segments = segments.reshape(-1, 1) # 10 classes, encoded as 0-9

                outputs = self.policy.mlp_extractor.forward_segmentation(rollout_data.observations)
                outputs = outputs[:, 1:, :]
                outputs = outputs.reshape(-1, 10) # 256*64x10

                # minimize cross entropy loss between segments and outputs
                segmentation_loss = F.cross_entropy(outputs, segments.flatten())
                segmentatioN_losses.append(segmentation_loss.item())
